...
`@zhouchengfeng/okay/vue` 是 `@zhouchengfeng/okay` 暴露的 Vue 3 子路径入口，提供 composables 和 ref 工具。通用业务逻辑仍建议优先从 `@zhouchengfeng/okay` 导入；只有需要 Vue 响应式生命周期或 `Ref` 时再使用该入口。

```bash
npm install @zhouchengfeng/okay
```

如果当前项目还没有安装 Vue，再安装 `vue`。已有 Vue 项目不需要重复安装框架运行时。
