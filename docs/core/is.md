...
| `isArray`        | `<T = unknown>(target: unknown) => target is T[]`                                        | `isArray<number>(value)`            |
...
| `isFunc`         | `<T extends (...args: never[]) => unknown>(target: unknown) => target is T`              | `if (isFunc(fn)) fn()`              |
| `isPromise`      | `<T = unknown>(target: unknown) => target is Promise<T>`                                 | `isPromise(fetch('/'))`             |
| `isDate`         | `(target: unknown) => target is Date`                                                    | `isDate(new Date()) // true`        |
| `isRegExp`       | `(target: unknown) => target is RegExp`                                                  | `isRegExp(/ok/) // true`            |
| `isMap`          | `<K = unknown, V = unknown>(target: unknown) => target is Map<K, V>`                     | `isMap<string, number>(value)`      |
| `isSet`          | `<T = unknown>(target: unknown) => target is Set<T>`                                     | `isSet(new Set()) // true`          |
| `isWeakMap`      | `<K extends WeakKey = WeakKey, V = unknown>(target: unknown) => target is WeakMap<K, V>` | `isWeakMap(new WeakMap())`          |
| `isWeakSet`      | `<T extends WeakKey = WeakKey>(target: unknown) => target is WeakSet<T>`                 | `isWeakSet(new WeakSet())`          |
