...
`@zhouchengfeng/okay/react` 是 `@zhouchengfeng/okay` 暴露的 React 子路径入口，当前聚焦 React ref 辅助方法，用于减少封装组件、透传 DOM ref 和兼容 callback ref 时的样板代码。

```bash
npm install @zhouchengfeng/okay
```

如果当前项目还没有安装 React，再安装 `react`。已有 React 项目不需要重复安装框架运行时。
