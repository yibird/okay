...
`okay` is a small TypeScript utility library. It publishes one npm package with multiple public entry points:

- `@zhouchengfeng/okay`: framework-agnostic utilities for type checks, async control, numbers, files, dates, arrays, and trees.
- `@zhouchengfeng/okay/vue`: Vue 3 composables and ref helpers exposed by the same package.
- `@zhouchengfeng/okay/react`: React ref helpers exposed by the same package.
...
`okay` is published as a single package. Vue and React helpers are exposed through subpath entries. If your project already uses Vue or React, no extra `okay` package is needed; just import the matching subpath. If the framework runtime is not installed yet, install it in your app:
...
Use the entry you need:

```ts
import { asyncTo, deferred } from '@zhouchengfeng/okay/async'
import { leaves } from '@zhouchengfeng/okay/coll'
import { formatBytes } from '@zhouchengfeng/okay/file'
...
The root entry `@zhouchengfeng/okay` is still available, but subpath imports keep non-bundled runtimes smaller.
...
This repository uses pnpm and keeps one lockfile: `pnpm-lock.yaml`.
