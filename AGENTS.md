...
This project is a TypeScript-based monorepo utility library.

The repository contains three packages:

- `core` - framework-agnostic utility functions
- `vue` - Vue-specific utilities and composables
- `react` - React-specific utilities and hooks

The `core` package must not depend on Vue, React, browser-specific APIs, or framework-specific implementations unless explicitly required.
...
Prefer placing logic in `core`.

Only place code in `vue` or `react` when framework integration is required.

Avoid duplicating business logic across packages.

Shared logic should live in `core`.
