...
const parseFnParams = (fn: Function): { count: number; hasRest: boolean } => {
  ...
  // Use fn.length as fast path for common cases
  // v8 ignore next 4 - fn.length === 2 is an optimization, hard to isolate in tests
  if (fn.length === 2) {
    const result = { count: 2, hasRest: false }
...
export function fastStableSort<T>(
  ...
  if (shouldUseKeySort(sortFn, options)) {
    // Validate options even for short arrays
