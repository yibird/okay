...
describe('fastStableSort', () => {
  ...
  it('covers fn.length === 2 fast path in parseFnParams', () => {
    // Test the fast path: if (fn.length === 2) at line 168-171
    // parseFnParams is only called in keySort path
    // Use a 2-param function with maxKey to force keySort
    const result = fastStableSort([3, 1, 2], (value: number, _index?: number) => value, {
      maxKey: 10,
    })
    expect(result).toEqual([1, 2, 3])
  })
