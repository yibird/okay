...
const readDefaultGroup = <T, Group extends IndexedMapKey>(item: T): Group | undefined => {
  // v8 ignore next 1 - items from array are always objects in practice
  if (!isObjectLike(item)) return undefined
  const group = (item as Record<string, unknown>).groupId
...
const mergePatch = <T>(current: T, patch: Partial<T>): T => {
  if (!isObjectLike(current) || !isObjectLike(patch)) {
    throw new TypeError('fastIndexedMap.update 的 patch 只支持对象数据')
  }

  return Object.assign({}, current, patch) as T
}
...
>(
  ...
  const setItem = (item: T) => {
    const id = getId(item)
    // v8 ignore next 3 - defensive check for invalid id types
    if (!isIndexKey(id)) {
  ...
  return {
    ...
    update: (id, patch) => {
      ...
      const next = typeof patch === 'function' ? patch(current) : mergePatch(current, patch)
