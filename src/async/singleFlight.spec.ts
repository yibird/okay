...
describe('singleFlight', () => {
  ...
  it('should reset pending promise after rejection', async () => {
    ...
    const func = vi.fn(() => {
      attempts++
      return attempts === 1 ? Promise.reject(new Error('failure')) : Promise.resolve('success')
    })
    const wrapped = singleFlight(func)
  ...
  it('should handle synchronous throw from func', async () => {
  ...
  it('should deduplicate by key when key option is provided', async () => {
    const func = vi.fn((id: number) => Promise.resolve(id * 2))
    const wrapped = singleFlight(func, { key: (id) => id })
    const [a, b, c] = await Promise.all([wrapped(1), wrapped(1), wrapped(2)])
