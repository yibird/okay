...
// Pre-compute digit char codes for faster comparison
const CHAR_CODE_0 = 48 // '0'
const CHAR_CODE_9 = 57 // '9'

const isAsciiDigit = (charCode: number) => charCode >= CHAR_CODE_0 && charCode <= CHAR_CODE_9

const countDigits = (text: string) => {
  let count = 0
  for (let i = 0; i < text.length; i++) {
    if (isAsciiDigit(text.charCodeAt(i))) {
      count++
    }
  }
  return count
...
const maskDigitChars = (
  ...
  for (let index = 0; index < chars.length; index++) {
    if (!isAsciiDigit(chars[index].charCodeAt(0))) continue
...
export const maskDigits = (
  ...
  // Convert to array once for masking
