...
describe('dateTimeRange', () => {
  it('should handle a single date', () => {
    ...
    const result = dateTimeRange(date)
  ...
  it('should handle an array of two dates', () => {
    ...
    const result = dateTimeRange(arr)
    expect(result).toEqual({
  ...
  it('should handle an object with start and end', () => {
    ...
    const result = dateTimeRange(obj)
    expect(result).toEqual({
      startDate: dayjs(obj.start).format('YYYY-MM-DD HH:mm:ss'),
      endDate: dayjs(obj.end).format('YYYY-MM-DD HH:mm:ss'),
    })
  })

  it('should throw if array has not exactly 2 dates', () => {
    expect(() => dateTimeRange(['2023-01-01'] as any)).toThrow(
      'Array input must contain exactly 2 dates',
    )
  })

  it('should throw if date is invalid', () => {
    expect(() => dateTimeRange('invalid-date')).toThrow('Invalid date(s) provided')
    expect(() => dateTimeRange({ start: '2023-01-01', end: 'invalid' })).toThrow(
      'Invalid date(s) provided',
    )
  })

  it('should throw if start date is after end date', () => {
    expect(() => dateTimeRange(['2023-01-31 10:01:01', '2023-01-01 09:01:01'])).toThrow(
