...
import { nextQuarter } from './nextQuarter'

describe('nextQuarter', () => {
  it('should return correct next quarter for Q1 (Jan-Mar)', () => {
    const { startTime, endTime } = nextQuarter('2023-02-15')
  ...
  it('should return correct next quarter for Q2 (Apr-Jun)', () => {
    const { startTime, endTime } = nextQuarter('2023-05-20')
  ...
  it('should return correct next quarter for Q3 (Jul-Sep)', () => {
    const { startTime, endTime } = nextQuarter('2023-08-10')
  ...
  it('should handle year transition for Q4 (Oct-Dec)', () => {
    const { startTime, endTime } = nextQuarter('2023-11-15')
  ...
  it('should use current date when no date is provided', () => {
    ...
    const expectedYear = nextQuarterIndex === 0 ? today.year() + 1 : today.year()
    const expectedStart = dayjs(new Date(expectedYear, nextQuarterIndex * 3, 1)).startOf('month')

    const { startTime } = nextQuarter()
  ...
  it('should handle different date formats', () => {
    ...
    expect(nextQuarter(new Date(testDate))).toMatchObject({
      startTime: dayjs(expected.start),
      endTime: dayjs(expected.end).endOf('day'),
    })

    expect(nextQuarter(testDate)).toMatchObject({
      startTime: dayjs(expected.start),
      endTime: dayjs(expected.end).endOf('day'),
    })

    expect(nextQuarter(dayjs(testDate))).toMatchObject({
      startTime: dayjs(expected.start),
      endTime: dayjs(expected.end).endOf('day'),
    })
  })
  ...
  it('should handle leap years correctly', () => {
    ...
    expect(nextQuarter('2023-12-01')).toMatchObject({
      startTime: dayjs('2024-01-01'),
      endTime: dayjs('2024-03-31').endOf('day'),
    })

    // Q4 2023 -> Q1 2024 (leap year)
    expect(nextQuarter('2023-11-15')).toMatchObject({
      startTime: dayjs('2024-01-01'),
      endTime: dayjs('2024-03-31').endOf('day'),
    })
  })
  ...
  it('should handle empty values as current date and reject invalid dates', () => {
    ...
    expect(() => nextQuarter('invalid-date')).toThrow('Invalid date provided')
    expect(nextQuarter('').startTime.isSame(expectedStart, 'day')).toBe(true)
    expect(nextQuarter(null).startTime.isSame(expectedStart, 'day')).toBe(true)
  })

  it('should return end time at end of day', () => {
    const { endTime } = nextQuarter('2023-05-15')
  ...
  it('should handle timezone correctly', () => {
    ...
    const { startTime: utcStart, endTime: utcEnd } = nextQuarter(utcDate)
    ...
    const { startTime: localStart, endTime: localEnd } = nextQuarter(localDate)
