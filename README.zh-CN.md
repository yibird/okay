...
`okay` 是一个 TypeScript 工具库，当前只发布一个 npm 包，并通过多个公开入口按需导入：

- `@zhouchengfeng/okay`：与框架无关的类型判断、异步控制、数字、文件、日期、数组和树工具。
- `@zhouchengfeng/okay/vue`：同一个包暴露的 Vue 3 composables 和 ref 工具入口。
- `@zhouchengfeng/okay/react`：同一个包暴露的 React ref 工具入口。
...
`okay` 现在只发布一个 npm 包。Vue 和 React 能力通过子路径入口暴露；如果你的项目已经安装了 Vue 或 React，不需要再安装额外的 `okay` 框架包，直接导入对应入口即可。如果项目还没有框架运行时，再按需安装：
...
根入口 `@zhouchengfeng/okay` 仍然可用，但推荐在对体积敏感的场景使用子路径导入。
...
仓库使用 pnpm，并只保留一个锁文件：`pnpm-lock.yaml`。
