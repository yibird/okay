...
import { prevQuarter } from './prevQuarter'

describe('prevQuarter', () => {
  it('should return correct previous quarter for Q1 (Jan-Mar)', () => {
    const { startTime, endTime } = prevQuarter('2023-02-15')
  ...
  it('should return correct previous quarter for Q2 (Apr-Jun)', () => {
    const { startTime, endTime } = prevQuarter('2023-05-20')
  ...
  it('should return correct previous quarter for Q3 (Jul-Sep)', () => {
    const { startTime, endTime } = prevQuarter('2023-08-10')
  ...
  it('should handle year transition for Q4 (Oct-Dec)', () => {
    const { startTime, endTime } = prevQuarter('2023-11-15')
  ...
  it('should use current date when no date is provided', () => {
    const today = dayjs()
    const currentQuarterIndex = Math.floor(today.month() / 3)
    const prevQuarterIndex = (currentQuarterIndex - 1 + 4) % 4
    const expectedYear = currentQuarterIndex === 0 ? today.year() - 1 : today.year()
    const expectedStart = dayjs(new Date(expectedYear, prevQuarterIndex * 3, 1)).startOf('month')

    const { startTime } = prevQuarter()
  ...
  it('should handle different date formats', () => {
    ...
    expect(prevQuarter(new Date(testDate))).toMatchObject({
      startTime: dayjs(expected.start),
      endTime: dayjs(expected.end).endOf('day'),
    })

    expect(prevQuarter(testDate)).toMatchObject({
      startTime: dayjs(expected.start),
      endTime: dayjs(expected.end).endOf('day'),
    })

    expect(prevQuarter(dayjs(testDate))).toMatchObject({
      startTime: dayjs(expected.start),
      endTime: dayjs(expected.end).endOf('day'),
    })
  })
  ...
  it('should handle empty values as current date and reject invalid dates', () => {
    ...
    expect(() => prevQuarter('invalid-date')).toThrow('Invalid date provided')
    expect(prevQuarter('').startTime.isSame(expectedStart, 'day')).toBe(true)
    expect(prevQuarter(null).startTime.isSame(expectedStart, 'day')).toBe(true)
  })

  it('should return end time at end of day', () => {
    const { endTime } = prevQuarter('2023-05-15')
  ...
  it('should handle timezone correctly', () => {
    ...
    const { startTime: utcStart, endTime: utcEnd } = prevQuarter(utcDate)
    ...
    const { startTime: localStart, endTime: localEnd } = prevQuarter(localDate)
  ...
  it('should handle edge cases at year boundaries', () => {
    // December 31st
    expect(prevQuarter('2023-12-31')).toMatchObject({
