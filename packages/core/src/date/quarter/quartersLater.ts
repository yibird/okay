 ...
 *
 * @param n 需要向后移动的季度数，必须为非负数。
 * @param date 参考日期，默认当前日期。
 * @returns 目标季度的开始和结束时间。
 * @throws 当 `n` 为负数时抛出错误。
 */
export function quartersLater(n: number, date?: dayjs.ConfigType): QuarterRange {
  if (n < 0) {
  ...
  const totalQuartersLater = currentQuarter + n
  const targetYear = d.year() + Math.floor(totalQuartersLater / 4)
  const targetQuarter = ((totalQuartersLater % 4) + 4) % 4
