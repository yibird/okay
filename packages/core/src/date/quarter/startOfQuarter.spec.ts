...
describe('startOfQuarter', () => {
  ...
  it('should handle string input', () => {
    const result = startOfQuarter('2020-06-01')
    expect(result.format('YYYY-MM-DD')).toBe('2020-04-01')
  })

  it('should handle Date object input', () => {
    const result = startOfQuarter(new Date(2023, 7, 15)) // August (Q3)
    expect(result.format('YYYY-MM-DD')).toBe('2023-07-01')
  })

  it('should handle timestamp input', () => {
    const timestamp = new Date(2023, 10, 15).getTime() // November (Q4)
    const result = startOfQuarter(timestamp)
    expect(result.format('YYYY-MM-DD')).toBe('2023-10-01')
  })
  ...
  it('should reject invalid dates', () => {
    expect(() => startOfQuarter('invalid-date')).toThrow('Invalid date provided')
  })
})
