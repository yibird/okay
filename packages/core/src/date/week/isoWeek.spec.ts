...
// test/isoWeek.spec.ts
import { describe, expect, it } from 'vitest'
import { isoWeek } from './isoWeek'

describe('isoWeek - ISO week number (full coverage)', () => {
  ...
  it('works with Date object (2015-12-31 -> ISO week 53)', () => {
    expect(isoWeek(new Date(2015, 11, 31))).toBe(53) // Dec 31, 2015
  })

  it('works with timestamp (number)', () => {
    const ts = new Date('2016-01-01').getTime()
    expect(isoWeek(ts)).toBe(53) // 2016-01-01 -> ISO week 53 (of 2015)
  })
