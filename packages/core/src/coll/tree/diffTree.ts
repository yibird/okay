...
const defaultNodeEqual = <T extends Record<PropertyKey, any>, ChildrenKey extends keyof T>(
  oldNode: T,
  newNode: T,
  childrenKey: ChildrenKey,
) => {
...
>(
  ...
  const idKey = options.idKey ?? ('id' as unknown as TreeDiffSelector<T, Id>)
  const childrenKey = options.childrenKey ?? ('children' as unknown as ChildrenKey)
