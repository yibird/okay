...
describe('mapTree (TS-friendly, performant)', () => {
  it('basic mapping preserves structure and order', () => {
    ...
    const mapped = mapTree<Node, { key: number; label: string }>(tree, (n) => ({
      key: n.id,
      label: n.name,
    }))
  ...
  it('supports custom childrenKey with correct typing', () => {
    ...
    const mapped = mapTree<Node, { uid: number; sub?: any }, 'sub'>(tree, (n) => ({ uid: n.id }), {
      childrenKey: 'sub',
    })
  ...
  it('filters nodes when mapper returns null but preserves children attached to nearest mapped ancestor', () => {
    ...
    const mapped = mapTree<Node, { id: number; name: string }>(tree, (n) =>
  ...
  it('handles deep trees without recursion (stack-safety)', () => {
    const depth = 500
    const make = (d: number): any => (d === 0 ? { id: 0 } : { id: d, children: [make(d - 1)] })
    const root = make(depth)
    const mapped = mapTree<{ id: number; children?: any }, { id: number }>([root], (n) => ({
      id: n.id,
    }))
    expect(mapped).toHaveLength(1)
