...
describe('diffTree', () => {
  ...
  it('isSamePath returns false for different-length paths', () => {
    // Move a node from depth-1 to depth-2 so isSamePath fires the length check
    const oldTree = [{ id: 'root', children: [{ id: 'child', children: [] }] }]
    const newTree = [
      { id: 'root', children: [{ id: 'mid', children: [{ id: 'child', children: [] }] }] },
    ]
    const result = diffTree(oldTree as any, newTree as any)
