...
export const maskText = (text: string, options: MaskOptions = {}): string => {
  ...
  if (ascii) {
    const visibleStart = text.slice(0, startCount)
    const visibleEnd = endCount > 0 ? text.slice(text.length - endCount) : ''
    return `${visibleStart}${maskChar.repeat(renderedMaskLength)}${visibleEnd}`
  }

  // Unicode path: iterate code points once
  const chars = Array.from(text)
  const visibleStart = chars.slice(0, startCount).join('')
